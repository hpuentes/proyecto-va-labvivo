...
await d3.csv("https://raw.githubusercontent.com/hpuentes/proyecto-va-labvivo/master/clean_base1.csv",(d) => {
  return {
...
{const inputType = d3.select(html`<input type="text" value="25" id="topWords"></input>`);
  ...
  main.variable(observer("viewof objvar")).define("viewof objvar", ["d3","html"], function(d3,html)
{
  ...
  <option value="colegios" >Institucion Educativa Distrital</option> 
  ...
  main.variable(observer("chart")).define("chart", ["objvar","distvar","data","d3","UserInput","textFrom","topNodesToGraph"], function(objvar,distvar,data,d3,UserInput,textFrom,topNodesToGraph)
{
  ...
  function setDataGraph(theDataGraph){
    ...
    tonoColor.set("Indiferente",colors[1])
    tonoColor.set("Triste",colors[3])
    tonoColor.set("Muy Triste",colors[8])
